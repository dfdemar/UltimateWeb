  ...
  .controller('TeampageCtrl', function ($scope) {
    ...
    $scope.$watch('teamStats', function(){
      if ($scope.teamStats && Object.keys($scope.teamStats).length > 2){
        $scope.ourBreakPercentageData = [{label:"D-Line Points", value:$scope.teamStats.goalSummary.ourDlineGoals}, 
                    {label:"O-Line Points", value:$scope.teamStats.goalSummary.ourOlineGoals}];
        $scope.theirBreakPercentageData = [{label:"D-Line Points", value:$scope.teamStats.goalSummary.theirDlineGoals}, 
                    {label:"O-Line Points", value:$scope.teamStats.goalSummary.theirOlineGoals}];
      }
    }, true);
