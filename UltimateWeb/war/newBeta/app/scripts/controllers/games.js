...
'use strict';
angular.module('newBetaApp').controller('GamesCtrl', function($scope, $q, $routeParams, $location, allGames, playerStats, gameStats) {
  ...
  $q.all([allGames, playerStats, gameStats]).then(function(responses) {
    ...
    psApi = responses[1];
    gsApi = responses[2];
    if ($routeParams.gameId && _(games).has($routeParams.gameId)) {
      scope.select(games[$routeParams.gameId]);
    } else {
